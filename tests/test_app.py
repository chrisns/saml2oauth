    ...
    def test_stores_saml_request_in_session(self, app_client, sample_saml_request):
        ...
        with patch("app.oauth.sso") as mock_sso:
            mock_sso.authorize_redirect.return_value = MagicMock(
                status_code=302, headers={"Location": "https://sso.example.com/authorize"}
            )

            # Use query_string dict to properly handle base64 chars like +
            app_client.get("/login", query_string={"SAMLRequest": sample_saml_request, "RelayState": "test-relay"})
...
class TestCallbackRoute:
    ...
    def test_successful_callback_creates_session(self, app_client, mock_secrets_manager, sample_oauth_token):
        ...
        with patch("app.oauth.sso") as mock_sso:
            ...
            with patch("app.push_user_info_to_scim"):
                app_client.get("/callback")

            with app_client.session_transaction() as sess:
                assert sess["signed_in"] is True
                assert sess["user"]["email"] == "test@example.gov.uk"

    def test_callback_extracts_all_user_claims(self, app_client, mock_secrets_manager, sample_oauth_token):
    ...
    def test_callback_rejects_unverified_email(self, app_client, mock_secrets_manager):
        ...
        unverified_token = {"userinfo": {"email": "unverified@example.com", "email_verified": False}}
...
class TestSamlResponseRoute:
    ...
    def test_generates_saml_response_for_signed_in_user(self, app_client, sample_user_claims, mock_secrets_manager):
    ...
    def test_includes_relay_state_when_provided(self, app_client, sample_user_claims, mock_secrets_manager):
    ...
    def test_escapes_relay_state_to_prevent_xss(self, app_client, sample_user_claims, mock_secrets_manager):
...
class TestBuildAutoPostHtml:
    ...
    def test_generates_form_with_saml_response(self):
        ...
        html = build_auto_post_html("https://sp.example.com/acs", "base64encodedsamlresponse", relay_state=None)
    ...
    def test_includes_relay_state_when_provided(self):
        ...
        html = build_auto_post_html(
            "https://sp.example.com/acs", "base64encodedsamlresponse", relay_state="https://return.url"
        )
    ...
    def test_escapes_relay_state_special_chars(self):
        ...
        html = build_auto_post_html(
            "https://sp.example.com/acs", "base64encodedsamlresponse", relay_state='"><img src=x onerror=alert(1)>'
        )
